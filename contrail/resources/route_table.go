...
func ResourceRouteTableCreate(d *schema.ResourceData, m interface{}) error {
	...
	if puuid_obj, ok := d.GetOk("parent_uuid"); ok {
		...
		if len(puuid_parts) > 1 {
			...
			if err != nil {
				return fmt.Errorf("[ResourceRouteTableCreate] retrieving Parent with uuid %s of type %s for resource %s (%s) - on %v (%v)", puuid_parts[1], puuid_parts[0], d.Get("name"), "RouteTable", client.GetServer(), err)
			}
...
func ResourceRouteTableRefsRead(d *schema.ResourceData, m interface{}) error {
	...
	base, err := client.FindByUuid("route-table", d.Id())
	if err != nil {
		return fmt.Errorf("[ResourceRouteTableRefsRead] Read resource route-table on %v: (%v)", client.GetServer(), err)
	}
	object := base.(*RouteTable)
	WriteRouteTableRefsToResource(*object, d, m)
	return nil
}
...
func ResourceRouteTableUpdate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ResourceRouteTableUpdate] Retrieving RouteTable with uuid %s on %v (%v)", d.Id(), client.GetServer(), err)
	}
...
func ResourceRouteTableRefsUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := client.FindByUuid("route-table", d.Id())
	if err != nil {
		return fmt.Errorf("[ResourceRouteTableRefsUpdate] Retrieving RouteTable with uuid %s on %v (%v)", d.Id(), client.GetServer(), err)
	}
	uobject := obj.(*RouteTable)
	UpdateRouteTableRefsFromResource(uobject, d, m)

	log.Printf("Object href: %v", uobject.GetHref())
	if err := client.Update(uobject); err != nil {
		return fmt.Errorf("[ResourceRouteTableRefsUpdate] Update of resource RouteTable on %v: (%v)", client.GetServer(), err)
	}
	return nil
}
