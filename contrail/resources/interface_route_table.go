...
func ResourceInterfaceRouteTableCreate(d *schema.ResourceData, m interface{}) error {
	...
	if puuid_obj, ok := d.GetOk("parent_uuid"); ok {
		...
		if len(puuid_parts) > 1 {
			...
			if err != nil {
				return fmt.Errorf("[ResourceInterfaceRouteTableCreate] retrieving Parent with uuid %s of type %s for resource %s (%s) - on %v (%v)", puuid_parts[1], puuid_parts[0], d.Get("name"), "InterfaceRouteTable", client.GetServer(), err)
			}
...
func ResourceInterfaceRouteTableRefsRead(d *schema.ResourceData, m interface{}) error {
	...
	base, err := client.FindByUuid("interface-route-table", d.Id())
	if err != nil {
		return fmt.Errorf("[ResourceInterfaceRouteTableRefsRead] Read resource interface-route-table on %v: (%v)", client.GetServer(), err)
	}
	object := base.(*InterfaceRouteTable)
	WriteInterfaceRouteTableRefsToResource(*object, d, m)
	return nil
}
...
func ResourceInterfaceRouteTableUpdate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ResourceInterfaceRouteTableUpdate] Retrieving InterfaceRouteTable with uuid %s on %v (%v)", d.Id(), client.GetServer(), err)
	}
...
func ResourceInterfaceRouteTableRefsUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := client.FindByUuid("interface-route-table", d.Id())
	if err != nil {
		return fmt.Errorf("[ResourceInterfaceRouteTableRefsUpdate] Retrieving InterfaceRouteTable with uuid %s on %v (%v)", d.Id(), client.GetServer(), err)
	}
	uobject := obj.(*InterfaceRouteTable)
	UpdateInterfaceRouteTableRefsFromResource(uobject, d, m)

	log.Printf("Object href: %v", uobject.GetHref())
	if err := client.Update(uobject); err != nil {
		return fmt.Errorf("[ResourceInterfaceRouteTableRefsUpdate] Update of resource InterfaceRouteTable on %v: (%v)", client.GetServer(), err)
	}
	return nil
}
