...
func ResourceNetworkIpamCreate(d *schema.ResourceData, m interface{}) error {
	...
	if puuid_obj, ok := d.GetOk("parent_uuid"); ok {
		...
		if len(puuid_parts) > 1 {
			...
			if err != nil {
				return fmt.Errorf("[ResourceNetworkIpamCreate] retrieving Parent with uuid %s of type %s for resource %s (%s) - on %v (%v)", puuid_parts[1], puuid_parts[0], d.Get("name"), "NetworkIpam", client.GetServer(), err)
			}
