...
func ResourceSubnetCreate(d *schema.ResourceData, m interface{}) error {
	...
	if puuid_obj, ok := d.GetOk("parent_uuid"); ok {
		...
		if len(puuid_parts) > 1 {
			...
			if err != nil {
				return fmt.Errorf("[ResourceSubnetCreate] retrieving Parent with uuid %s of type %s for resource %s (%s) - on %v (%v)", puuid_parts[1], puuid_parts[0], d.Get("name"), "Subnet", client.GetServer(), err)
			}
...
func ResourceSubnetRefsRead(d *schema.ResourceData, m interface{}) error {
	...
	base, err := client.FindByUuid("subnet", d.Id())
	if err != nil {
		return fmt.Errorf("[ResourceSubnetRefsRead] Read resource subnet on %v: (%v)", client.GetServer(), err)
	}
	object := base.(*Subnet)
	WriteSubnetRefsToResource(*object, d, m)
	return nil
}
...
func ResourceSubnetUpdate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ResourceSubnetUpdate] Retrieving Subnet with uuid %s on %v (%v)", d.Id(), client.GetServer(), err)
	}
	uobject := obj.(*Subnet)
...
func ResourceSubnetRefsUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := client.FindByUuid("subnet", d.Id())
	if err != nil {
		return fmt.Errorf("[ResourceSubnetRefsUpdate] Retrieving Subnet with uuid %s on %v (%v)", d.Id(), client.GetServer(), err)
	}
	uobject := obj.(*Subnet)
	UpdateSubnetRefsFromResource(uobject, d, m)

	log.Printf("Object href: %v", uobject.GetHref())
	if err := client.Update(uobject); err != nil {
		return fmt.Errorf("[ResourceSubnetRefsUpdate] Update of resource Subnet on %v: (%v)", client.GetServer(), err)
	}
	return nil
}
